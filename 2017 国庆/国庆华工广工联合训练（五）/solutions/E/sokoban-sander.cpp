...
bool is_goal[MAXLOC+1], done[MAXLOC+1][MAXLOC+1][MAXLOC];
int  exitloc;
...
//  cout << "non-wall count: " << nloc << " boxes: " << nbox << endl;
  //if(nbox == 0) return 0;
  if(nbox <= 0) boxloc[0] = MAXLOC;
  if(nbox <= 1) boxloc[1] = MAXLOC;
  is_goal[MAXLOC] = false;
  ...
  for(moves=0;!qto.empty();moves++) { 
    ...
    while(!q.empty()) { // breadth-first search
      ...
      //if( is_goal[p.box1] && is_goal[p.box2] && p.player == exitloc ) goto solved;
      int buttons = is_goal[p.box1] + is_goal[p.box2];
      if (buttons >= nbut && p.player == exitloc) goto solved;
      ...
      for(int e=0;e<4;++e) {
        ...
        if(next.box1==WALL || next.box2==WALL || (next.box1 != MAXLOC && next.box1==next.box2)) continue;
